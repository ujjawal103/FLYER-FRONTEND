...
const FinishRide = ({setFinishRidePannel , setLoading , setMessage}) => {
    ...
    useEffect(() => {
      if (Object.keys(currRide).length > 0 && currRide.status === "completed") {
        navigate("/ride-completed", { state: { userType: "pilot" } });
      }
    }, [currRide]);

  return (
    <div>
       <h2 className='text-xl font-medium'>Finish this ride</h2>
        ...
        <div className='flex justify-center items-center flex-col gap-3'>
           ...
           <div className='mt-6 w-full flex flex-col gap-3'>
            <button onClick={finishRide} className='w-full bg-green-700 text-white p-2 font-semibold rounded flex justify-center'>Finish Ride</button>
            <p className='mt-2 text-red-500 text-xs'>click on finish ride button if you have completed the payment</p>
           </div>
        </div>
    </div>
  )
