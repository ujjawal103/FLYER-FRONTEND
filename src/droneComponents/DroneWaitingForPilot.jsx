...
const DroneWaitingForPilot = ({setWaitingForPilot , rideConfirmed , rideOtp}) => {
    ...
    const handleCancel = async () =>{
       ...
       try{
        ...
        if(response.status === 200){
            ...
            toast.success("Schedule Cancelled Successfuly");
        }
       }catch(err){
        setLoading(false);
        setMessage("");
  ...
  return (
     ...
     <div>
       <h2 className='text-xl font-medium'>Waiting For The Drone to pickup</h2>
        ...
        <div className='flex justify-center items-center flex-col gap-3 mt-3'>
            ...
            <div className='w-full'>
                ...
                <div className='w-full flex justify-between items-center px-10 py-4 bg-gray-100 rounded-lg mt-4 mb-3'>
                    <h5>Share this OTP to Drone</h5>
                     <h4 className='font-semibold text-2xl'>{rideOtp}</h4>
                </div>
                <button onClick={handleCancel} className='w-full bg-red-600 text-white p-2 font-semibold rounded mb-2'>Cancel Delivery</button>
            </div>
...
export default DroneWaitingForPilot
